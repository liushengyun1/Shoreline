...
def generate_cropped_data(image, graph, crop_size=448, overlap_size=0):
    """
    将图像裁剪成小尺寸图像，并处理对应的图结构

    Args:
        image: 输入图像，形状为 (height, width, 3) 的 .tif 图像
        graph: 简化的 networkx.Graph，节点坐标为 (x, y)
        crop_size: 裁剪后的图像尺寸
        overlap_size: 裁剪时的重叠尺寸

    Returns:
        cropped_image_list: 裁剪后的图像列表
        cropped_graph_list: 对应的裁剪后的图结构列表
    """
...
def merge_dense_keypoints(graph, merge_dist=5):
    ...
    Args:
        ...
        merge_dist (float): 合并的最大距离

    Returns:
        合并后的图
    """
    done_set = []  # 记录已搜索的节点
    match_dict = {}  # 记录需要合并的点及其质心
...
def separate_keypoints_near_edge(graph, image_shape, edge_width=10, min_dist=10):
    """
    分离边缘附近的端点，使这些点之间的距离不小于最小距离

    Args:
        graph (networkx.Graph): 输入图
        image_shape (Tuple(int, int)): 图像形状
        edge_width: 处理区域宽度
        min_dist (float): 两点之间的最小距离
